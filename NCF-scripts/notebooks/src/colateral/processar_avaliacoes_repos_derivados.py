...
def process_repos_counterparty(counterparty, resolved_path, today):
    ...
    if file_extension == '.pdf' and counterparty == "BBVA":
        return process_repos_bbva(resolved_path, today)
    else:
...
def atualizar_bd_avaliacoes_contrapartes_repos(file_paths: dict,date: str,excel_dir: str, db_paths: str, con, clear: bool = False,nome_tabela: str = "avaliacoes_contrapartes") -> pd.DataFrame:
    ...
    for counterparty, paths in file_paths.items():
        ...
        try:
            df = process_repos_counterparty(counterparty, resolved_path, today)
...
def atualizar_bd_avaliacoes_igcp_repos(df: pd.DataFrame,today, con, nome_tabela: str = "avaliacoes_igcp_repos", salvar: bool = False) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df = df[["Transacao","Contraparte","Position_date",
                "Opening_date","Value_date","Expiry_date",
                "ISIN","Start_cash","Nominal","Clean_price",
                "Dirty_price","Accrued_interest_cash","Accrued_interest_bond","MTM"]]
...
def atualizar_precos_igcp_repos(df: pd.DataFrame, con, nome_tabela: str = "preco_instrumentos_coll", salvar: bool = False):
    
    df=df.drop(columns={"Row Id"})
    df=df.rename(columns = {"Spread Bid-Offer p.b":"Spread_Bid_Offer","Mid Price":"Mid_price","Security Identifier":"Security_identifier"})
